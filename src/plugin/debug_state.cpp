...
void DebugStateTracker::NotifyPaused(PauseReason reason)
{
    ...
    {
        // Уже стоим на паузе (второй коллбэк той же остановки). x64dbg может
        // прислать несколько уведомлений на одну физическую остановку, и
        // порядок их прихода не гарантирован (например, CB_PAUSEDEBUG с
        // UserPause может опередить более точный CB_STEPPED с Step). Поэтому
        // более приоритетная (более определённая) причина вытесняет менее
        // приоритетную, а не наоборот — иначе точная причина терялась бы
        // из-за случайно пришедшего раньше грубого уведомления о паузе.
        // Поколение не трогаем: это по-прежнему та же самая физическая пауза.
        snapshot_.reason = reason;
